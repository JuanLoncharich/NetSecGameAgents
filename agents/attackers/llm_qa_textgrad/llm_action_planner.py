...
class LLMActionPlanner:
    def __init__(self, model_name: str, goal: str, memory_len: int = 10, api_url=None,
                 ...
                 use_self_consistency: bool = False, config_file: str = None, custom_instructions: str = None):

        if config_file:
            self.config = ConfigLoader.load_config_from_file(config_file)
        elif config:
            self.config = config
        else:
        ...
        if self.custom_instructions:
            self.instructions = self.custom_instructions
        else:
            self.update_instructions(goal)
